    ...
    switch (packet_type)
    {
        ...
        case PUBLISH:
        {
            ...
            if (msg.qos != QOS0)
            {
                ...
                {
#ifdef CJY_DEBUG
                    printf("mqtt cycle published failed !\n");
#endif
        ...
        case PUBREL:
        {
            ...
            {
#ifdef CJY_DEBUG
                printf("mqtt cycle pubreal failed !\n");
#endif
                goto exit; // there was a problem
            }
...
#ifdef CJY_DEBUG
        printf("mqtt cycle keepalive failed !\n");
#endif
        rc = FAILURE;
    }
...
void MQTTRun(void* parm)
{
	...
	while (1)
	{
		...
		MutexLock(&c->mutex);
#endif
		TimerCountdownMS(&timer,10500); /* Don't wait too long if no traffic is incoming */
		cycle(c, &timer);
#if defined(MQTT_TASK)
		MutexUnlock(&c->mutex);
#endif
