...
uint8_t gprs_buf[512] = {0};   //GSM接受缓冲区，全局变量
...
{
    UART4_Rx(gprs_buf,sizeof(gprs_buf));
}
...
static uint8_t Gsm_AT_SELF(uint8_t * buff)
{
	if(Gsm_SendAndWait(buff,(uint8_t *)"OK",1,RETRY_NUM,1000))
        return 1;
    else
        return 0;
}
...
#ifdef CJY_DEBUG
    printf("AT+CGSOCKCONT=1,IP,ctm2m\n");	
...
Gsm_SendAndWait((uint8_t *)"AT+CGDCONT?\r\n",(uint8_t *)"+CREG: ",1,RETRY_NUM,1000);
printf("%s\n",gprs_buf);
Gsm_SendAndWait((uint8_t *)"AT+CPSI?\r\n",(uint8_t *)"+CREG: ",1,RETRY_NUM,1000);
printf("%s\n",gprs_buf);
Gsm_SendAndWait((uint8_t *)"AT+SIMCOMATI\r\n",(uint8_t *)"+CREG: ",1,RETRY_NUM,1000);
printf("%s\n",gprs_buf);
Gsm_SendAndWait((uint8_t *)"AT+CUSB\r\n",(uint8_t *)"+CREG: ",1,RETRY_NUM,1000);
printf("%s\n",gprs_buf);*/
 ...
 *模块开机接口初始化
 *************************************************************/
static uint8_t gsm_failed_cnt = 0;
static void restartGSM()
{
        BSP_SIM7600_PWR_SET(0);
